...
Чтобы расширить выбор остановок отправления и прибытия, которые которые будут рассматриваться программой как возможные маршруты, можно группировать остановки.
Например:  
...
#### Исключение группировки
- Если выбран маршрут от остановки к остановке внутри одной группы, остальные остановки груааы игнорируются во избежании рекурсивного составления расписания от каждой к каждой. Логика реализована в методе `StopGroup.get_group_by_stop_name`.
- Если 2 или более остановок отправления из группы оказываются на одном маршруте каког-то автобуса для него исключаются остановки кроме названной остановки отправления или если ее нет в этом маршруте, называется последняя из группы, по пути его следования. Для того, чтобы избежать объявления одного автобуса на последовательных остановках. Логика реализована `ПОКА НИГДЕ`.
