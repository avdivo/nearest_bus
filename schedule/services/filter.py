    ...
    class BusStorage:
        ...
        def save(self, route: Dict) -> bool:
            ...
            if self.best_priority > priority:
                ...
                self.storage = {}
                self.best_priority = (
                    priority  # Запоминаем лучший приоритет для автобуса
                )

            # Приоритет больше или равен сохраненным
            saved_route = self.storage.setdefault(
                part, {}
            )  # Получение или создание маршрута
            saved_score = saved_route.get(
                "score", 10000
            )  # Получение сохраненных баллов
