...
for bus, directions in data.items():  # Номер автобуса и названия маршрутов
    ...
    for direction, bus_stops in directions.items():  # Название маршрута и список остановок на нем
        ...
        try:
            id = merge_bus_stops(bus_stops[bus_stop_start]['id'])  # id остановки
            # Добавление конечной остановки в БД +++++++++++++++++++
            bus_stop_start_obj = add_bus_stop(bus_stop_start, id, True)
            id = merge_bus_stops(bus_stops[bus_stop_end]['id'])  # id остановки
            # Добавление конечной остановки в БД +++++++++++++++++++
        ...
        for i, (bus_stop, rest) in enumerate(bus_stops.items()):  # Остановка и остальные данные (в т.ч. расписание)
            ...
            external_id = merge_bus_stops(rest['id'])  # id остановок по Миноблавтотранс
