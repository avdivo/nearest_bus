    ...
    def execute(self):
        ...
        if self.stage == 1:
            ...
            if self.key_name in week:
                ...
                # Получаем объекты остановок отправления с одним названием


                routers = route_analysis(start, finish)  # Получаем маршруты
                start_obj = {(obj["start"], obj["final_stop_finish"])
                             for obj in routers if obj["start"].name == start and obj["start"]}  # Выбираем нужные
                ...
                # и отправляем в тг
                for start, final_stop_finish in start_obj:
                    text = ""
                    text += f"🚥 Остановка {start.name}\n(в сторону {final_stop_finish.name})\n"

                    # Получаем расписание на остановке и разбираем
                    schedule = full_schedule(start)  # Dict[Bus, List[datetime.time]]
                    # print(text)
                    # print(schedule)
                    # for bus in schedule:
                    #     text += f"Автобус №{bus.number}\n" + ", ".join(schedule["bus"])
