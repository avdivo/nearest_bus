...
def answer_to_alisa(request_body, user=None):
    """Функция получает все сообщения от Алисы. Или от ТГ.
    Определяет их статус и выполняет
    действия в соответствии с ним.
    Принимая запрос от ТГ обрабатывает его так же, как от Алисы
    (не подозревая о подмене),
    при этом в request_body должны приходить нужные данные:
    request_body['session']['application']['application_id'] - идентификатор устройства
     (для тг свой идентификатор)
    request_body['request']['original_utterance'] - текст запроса.
    Принимает тело запроса от Алисы или ТГ.
    user - объект пользователя Алисы, если не передан, то определяется в программе
    """
    ...
    def datetime_bus(time) -> datetime:
        ...
        )  # Дата и время отправления автобуса
        bus_datetime = bus_datetime.astimezone(
            current_timezone
        )  # Преобразуем в текущий часовой пояс
        ...
        if minutes < 60:
            minutes_word = (
                ...
                if minutes % 10 == 1 and minutes % 100 != 11
                    ...
                    "минуты"
                    if 1 < minutes % 10 < 5
                    and (minutes % 100 < 10 or minutes % 100 >= 20)
                    else "минут"
                )
            )
    ...
    def say_schedule(start, end):
        ...
        try:
            ...
            return (
                f"Между остановками {start} и {end} нет прямого автобуса. "
        ...
        user.set_parameters(
            [start, end], "stops"
        )  # Запоминаем начальную и конечную остановки для повтора
        limit = 2
    ...
    if not out:
        ...
        if out[0] in commands:
            if out[0] == "Что ты умеешь":
                return (
                    "Я подскажу вам, какие автобусы в ближайшее время идут по названному маршруту в Слуцке."
                ...
                return (
                    "Чтобы узнать расписание назовите маршрут (с какой остановки на какую ехать)."
                ...
                words = [
                    ...
                    "Было приятно помочь.!",
                    "Ваша признательность – лучшая награда.",
                ...
                if user.get_parameters("time"):
                    stops = user.get_parameters("stops")
                    schedule = answer_by_two_busstop(
                        stops[0], stops[1]
                    )  # Расписание автобусов на остановке
                    time_bus = datetime.fromisoformat(
                        user.get_parameters("datetime")
                    )  # Дата и время отправления автобуса
                    ...
                    if time_now > time_bus:
                        ...
                        search = datetime.strptime(
                            user.get_parameters("time"), "%H:%M"
                        ).time()
