...
def greetings(user: AlisaUser) -> str:
    ...
    вопрос задается прямо. У бывалых, с именем, можно разообразить вопрос.
    ...
    """
    created_at = getattr(user, 'previous_last_update', user.last_update)  # Когда был последний раз
    ...
    if created_at and date.date() != created_at.date():
        ...
        hello = query_openrouter(
            ...
            prompt_suffix = f"Сейчас {part_of_day}\n{f'Пользователь {user_name}' if user_name else ''}",
        )
